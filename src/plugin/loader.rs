use super::api::{InspectorPlugin, PluginConstructor, PluginError, PluginResult, PLUGIN_CONSTRUCTOR_SYMBOL};
...
impl PluginLoader {
    ...
    pub fn default_plugin_dir() -> PluginResult<PathBuf> {
        let home = dirs::home_dir()
            .ok_or_else(|| PluginError::InitializationFailed("Could not determine home directory".to_string()))?;
    ...
    pub fn load_library(&self, library_path: &Path, manifest: PluginManifest) -> PluginResult<LoadedPlugin> {
        ...
        unsafe {
            ...
            let constructor: libloading::Symbol<PluginConstructor> = library
                ...
                .map_err(|e| PluginError::Invalid(format!(
                    "Plugin does not export '{}': {}", PLUGIN_CONSTRUCTOR_SYMBOL, e
                )))?;
